...
public class FTwo implements Filter<List<CompanyIssuer>> {
    ...
    public FTwo(CompanyIssuerRepository companyIssuerRepository, DayPriceRepository dayPriceRepository) {
    ...
    public List<CompanyIssuer> execute(List<CompanyIssuer> input) throws IOException {
        List<CompanyIssuer> companies = new ArrayList<>();

        for (CompanyIssuer companyIssuer : input) {
            if (companyIssuer.getLastUpdated() == null) {
                for (int i = 1; i <= 10; i++) {
                    int temp = i - 1;
